...
+!prima_mossa /* Siamo primi nelle mosse */
...
+end_game(Winner, Win, Step)[source(Source)]
    ...
    <-  .print("The game ended with a winner: ", Winner, " all'iterazione ", Step).

+end_game(Winner, Win, Step)[source(Source)]
    : Win == "true" & Winner == "tie" & arbiter(Arbiter) & Arbiter == Source
    <- .print("The game ended with a tie all'iterazione ", Step, " DOBBIAMO RIPETERE");
        !repeat_game.
